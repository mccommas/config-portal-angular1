(function () {
    "use strict";
    configAppControllers.controller('SubmitOfferReviewModal', ["$scope", "$state", "toastr", "$timeout", "$uibModalInstance", "$location", "Restangular",
        function ($scope, $state, toastr, $timeout, $uibModalInstance, $location, Restangular) {
            ...
            $scope.post_submit_for_review = function () {
                serverSubmit.all("gatewayStatus/extend_offer").post('review').then(function (data) {
                    $uibModalInstance.close(true);
                    toastr.success('Extend Offer Activity Submitted for Review', 'Success', {
                        allowHtml: true
                    });
